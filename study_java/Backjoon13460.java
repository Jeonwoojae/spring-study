...
public class Main {
    ...
    static final int LIMIT = 10;  // 이동할 수 있는 최대 횟수
    ...
    // 이동 경로가 같은 방향을 연속해서 두번 이상, 갔던 방향의 반대 방향이 아닌지 확인하여 유효한 이동 경로만 선택
    ...
    public static void main(String args[]) {
        ...
        for (int k=0; k<(1<<(LIMIT*2)); k++) {
            ...
            if (!valid(dir)) continue;  // 이동 경로가 유효하지 않으면 건너뛴다.
            for (int i=0; i<n; i++) {
                a[i] = map[i].toCharArray();
            }
            int cur = check(a, dir);  // 이동 경로가 유효한지 확인한다.
            if (cur == -1) continue;  // 유효하지 않으면 건너뛴다.
            if (ans == -1 || ans > cur) ans = cur;  // 최소 이동 횟수 갱신
        }
